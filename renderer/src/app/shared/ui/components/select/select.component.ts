...
import { AfterViewInit, ChangeDetectionStrategy, Component, computed, contentChildren, effect, ElementRef, HostBinding, input, output, signal, viewChild } from '@angular/core';
...
export class SelectComponent implements AfterViewInit {
    ...
    protected selectedEl = viewChild.required<ElementRef<HTMLElement>>('selected');
    protected optionsEl  = viewChild.required<ElementRef<HTMLElement>>('options');
    ...
    private onOptionSelected(option: SelectOptionComponent): void {
        ...
        if(this.isMultiple) {
            ...
            if(selected.includes(option)) {
                if(selected.length < 2 && !this.clearable()) {
                    return;
                }
